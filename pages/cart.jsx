  ...
  return (
    <>
      ...
      <main className="container" style={{padding:40}}>
        <h1>Your cart</h1>
        {items.length === 0 ? (
          <div className="small">Your cart is empty. <Link href="/shop"><a>Shop now</a></Link></div>
        ) : (
          <>
            <ul>
              {items.map(it=>(
            ...
            <div style={{marginTop:16,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              ...
              <div>
                <button className="btn" onClick={clear} style={{marginRight:8}}>Clear</button>
                <button className="primary-cta" onClick={()=> alert('Checkout not configured')}>Checkout</button>
              </div>
            </div>
          </>
        )}
